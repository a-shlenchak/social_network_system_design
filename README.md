...
Сезонность - присутствует. С мая по сентябрь рост трафика в 2,5-3 раза
...
При просмотре поста же мы будем выводить всю информацию: название + текст + фотографии, а также оценки и комментарии (будет отражено далее).
...
Для ленты загружается 10 постов за раз. С учетом того, что сверху в ленте отображаются новые посты и пользователь, просматривающий ленту 1-2 раза в день будет, скорее всего, заинтересован в чтении новых постов в большинстве случаев, дополнительную подгрузку следующей "страницы" ленты учитывать не будем.
...
```
Traffic (write): 1 500 * 600 = 900 000 b/s = 900 kb/s или 0.9 Mb/s
```
...
Создание локаций происходит при создании поста - 0.15 раз в день для пользователя. Этот показатель будет постепенно сходить на нет, тк все чаще при создании постов пользователя будут находить уже имеющуюся локацию. Так, что в конечном счете нагрузка на запись будет стремиться к 0.
